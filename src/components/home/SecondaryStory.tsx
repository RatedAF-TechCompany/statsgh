...
export const SecondaryStory = ({ article, showImage = true, variant = "default" }: SecondaryStoryProps) => {
  ...
  if (variant === "horizontal") {
    return (
      <Link href={href} className="block group">
        <article className="py-3 border-b border-border cursor-pointer">
          <div className="flex gap-3">
            ...
            {showImage && article.hero_image_url && (
              <img src={article.hero_image_url} alt="" className="w-20 h-20 object-cover flex-shrink-0" />
            )}
          </div>
        </article>
      </Link>
  ...
  if (variant === "compact") {
    return (
      <Link href={href} className="block group">
        <article className="py-2.5 border-b border-border cursor-pointer">
  ...
  return (
    <Link href={href} className="block group">
      <article className="cursor-pointer">
        ...
        {article.published_at && (
          <p className="font-sans text-xs text-muted-foreground mt-2">{getTimeAgo(article.published_at)}</p>
        )}
      </article>
    </Link>
  );
